...
Status: RPL-001 through RPL-012 implemented; curated library, caching, and portability work remains open
...
The repository has four rules CLIs, a Vite simulator, and a [replay core library](../../replay/README.md) with schema validation, deterministic navigation, presentation transitions, PGN import, synchronized move lists, and 20 shared technical fixtures. The public viewer now uses a Three.js board with an accessible deterministic fallback and local PGN import; the curated library and retro compilation remain planned work. Web V1 is Phases 1 and 2; Phase 3 is the later portability proof. The completed core is exposed through the public viewer while the simulator remains independently reachable. No deployment is part of the core tickets.
