...
test('the curated library contains attributed canonical replays with varied behavior', () => {
  ...
  assert.deepEqual(CURATED_REPLAYS.map((entry) => entry.id), [
    'immortal-game',
    'opera-game',
    'evergreen-game',
    'game-of-century',
    'fischer-spassky-game-six',
    'kasparov-topalov-1999',
  ]);
  assert.equal(CURATED_REPLAYS.some((entry) => entry.replay.moves.some((move) => move === 'e1g1')), true);
  assert.equal(CURATED_REPLAYS.some((entry) => entry.replay.moves.some((move) => move === 'e1c1')), true);
  assert.equal(CURATED_REPLAYS.some((entry) => entry.replay.metadata?.result === '0-1'), true);
  assert.equal(CURATED_REPLAYS.some((entry) => entry.replay.metadata?.source_url?.toString().includes('wikipedia.org')), true);
});

test('curated lookup returns isolated replay copies', () => {
  assert.equal(curatedReplayById('missing'), null);
  const first = cloneCuratedReplay('immortal-game');
  const second = cloneCuratedReplay('immortal-game');
...
test('the public viewer defaults to the first curated replay', () => {
  assert.equal(DEFAULT_REPLAY_ID, 'immortal-game');
